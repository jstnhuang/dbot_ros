...
#include "dbot_ros/util/camera_info_camera_provider.h"
...
std::shared_ptr<dbot::ParticleTracker> ParticleTrackerBuilder::Build()
{
    ...
    // gpu only parameters
    nh_.getParam(pre + "gpu/use_custom_shaders",
                 params_obsrv.use_custom_shaders);
    nh_.getParam(pre + "gpu/vertex_shader_file",
    ...
    nh_.getParam(pre + "center_object_frame",
...
void BuildOri(const ros::NodeHandle& nh,
